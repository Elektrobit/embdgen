...
def get_uid_and_gid(fr: FakeRoot, file: Path) -> str:
    res = fr.run(["stat", "-c", "%u:%g", file], stdout=subprocess.PIPE, encoding="utf-8")
...
def test_simple(tmp_path: Path):
    ...
    fr.run(["touch", file1, file2])
    fr.run(["chown", "123:456", file1])
    fr.run(["cp", "-p", file1, file3])
